...
| preventDefault()           | Function     | 只读   | 取消事件的默认行为。如果cancelable是true，则可以使用这个方法（或理解为阻止同一个事件的其他监听函数被调用） |
...
| timeStamp                  | 毫秒时间戳        | 只读   | 事件发生的时间                                  |
| isTrusted                  | Boolean      | 只读   | 该事件是否为真实用户触发。 用户触发的事件返回`true`，脚本触发的事件返回`false` |
